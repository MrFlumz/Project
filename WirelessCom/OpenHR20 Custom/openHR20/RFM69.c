...
void rfm69_init(uint16_t freqBand, uint8_t nodeID, uint8_t networkID)
{
    ...
    DDRD |= 1<<2;       // setting SS as output
    PORTD |= 1<<2;      // setting slave select high
    DDRD &= ~(1<<1);  // setting interrupt pin input. no problem if not given
    //PORTD &= ~(1<<1); // setting pull down. because rising will cause interrupt. external pull down is needed.
