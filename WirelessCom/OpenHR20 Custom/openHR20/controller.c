...
//defines fra eepron config

#define valve_max 100
#define valve_min 0 
#define PID_interval 240/5
#define I_max_credit 40
#define	I_credit_expiration 30
#define temp_boost_setpoint_diff 50
#define temp_boost_error 30
#define temp_boost_time_heat 15
#define temp_boost_time_cool 64
#define temp_boost_tempchange 5
#define temp_boost_hystereses 10
#define I_Factor 32
#define P3_Factor 33
#define P_Factor 8
#define valve_center 45
#define valve_hysteresis 64
    ...
    if (PID_force_update>0) { 
        ...
        bool updateNow=(temp!=CTL_temp_wanted_last);
		  if (updateNow||(PID_update_timeout == 0)) {
            PID_update_timeout = (PID_interval * 5); // new PID pooling
			uint8_t new_valve;
            if (temp>TEMP_MAX) {
                new_valve = valve_max;
            } else {
                new_valve = pid_Controller(calc_temp(temp),update_average_temp()/2,valveHistory[0],updateNow);
			...
			{	
				...
				for(i=VALVE_HISTORY_LEN-1; i>0; i--) {
					if (updateNow || (new_valve <= valve_max) || (new_valve >= valve_min)) {
...
static uint8_t pid_Controller(int16_t setPoint, int16_t processValue, uint8_t old_result, bool updateNow)
{
...

  {
	  ...
	  if (updateNow) {
		  ...
		  #if BOOST_CONTROLER_AFTER_CHANGE
			  if ((abs(setPoint-CTL_temp_wanted_last)>=temp_boost_setpoint_diff) // change of wanted temp large enough to start boost (0,5°C)
				&& (absErr>=(int16_t)temp_boost_error)) {  // error large enough to start boost (0,3°C)
  ...
  #if BOOST_CONTROLER_AFTER_CHANGE
	if (PID_boost_timeout > 0) {
		if ((abs(error16)<=(int16_t)(temp_boost_error-temp_boost_hystereses))) {  // error <= temp_boost_error-temp_boost_hystereses
			...
			if (error16>0) {
			  return valve_max;   //boost to max, no PID calculation
			} else {
			  return valve_min;   //boost to min, no PID calculation
			}
		}
	}
  #endif
